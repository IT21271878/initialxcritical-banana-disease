...
async def predict_image(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Preprocess for MobileNetV2 — using NumPy to avoid Keras 3 tensor op issues
        image = image.resize((224, 224))
        img_array = np.array(image, dtype=np.float32) / 255.0  # Rescale in NumPy
        img_array = np.expand_dims(img_array, axis=0)           # Add batch dim in NumPy
